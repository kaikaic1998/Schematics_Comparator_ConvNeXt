...
def transform (image_size):
    # Get ConvNeXt normalization values
...
# model
model = SiameseNetwork(freeze_parameter).to(device)
...
settings_name_for_file = ("ConvNeXt_" + "FreezeParameter" + str(freeze_parameter) +
            "_Resize" + str(resize) +
            "_Epochs" + str(epochs) +
            "_Batch" + str(batch) +
...
settings_name_for_plot_title = ("Model used: ConvNeXt" + 
